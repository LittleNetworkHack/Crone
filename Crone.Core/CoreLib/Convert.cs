...
public static partial class CoreLib
{
    ...
    public static T ConvertTo<T>(object value, T defaultValue)
    {
        ...
        return (T)ConvertTo(value, typeof(T), defaultValue);
    }
    ...
    public static object ConvertTo(object value, Type destType, object defaultValue)
    {
        try
        {
            ...
            if (destType == typeof(object) || srcType == destType || destType.IsAssignableFrom(srcType))
                return value;
            ...
            // We treat T and T? as same type
            destType = Nullable.GetUnderlyingType(destType) ?? destType;

            // Switch hack to avoid too many if for brevity
            value = destType switch
            {
                var type when type.IsEnum => ToEnum(value, type),
                var type when type == typeof(bool) => ToBoolean(value),
                var type when type == typeof(char) => ToChar(value),
                var type when type == typeof(string) => ToString(value),

                var type when type == typeof(float) => ToSingle(value),
                var type when type == typeof(double) => ToDouble(value),
                var type when type == typeof(decimal) => ToDecimal(value),

                var type when type == typeof(byte) => ToByte(value),
                var type when type == typeof(short) => ToInt16(value),
                var type when type == typeof(int) => ToInt32(value),
                var type when type == typeof(long) => ToInt64(value),

                var type when type == typeof(sbyte) => ToSByte(value),
                var type when type == typeof(ushort) => ToUInt16(value),
                var type when type == typeof(uint) => ToUInt32(value),
                var type when type == typeof(ulong) => ToUInt64(value),

                var type when type == typeof(byte[]) => ToByteArray(value),
                var type when type == typeof(DateTime) => ToDateTime(value),
                //var type when type == typeof(TimeSpan) => ToTimeSpan(value),
        ...
        {
#if DEBUG
            // We cant have this throw, so in debug we need to know
    ...
    public static DateTime? ToDateTime(long value) => value > 0 ? (DateTime?)new DateTime(value) : null;
    ...
    public static Guid? ToGuid(string value)
    {
        try
        {
            return string.IsNullOrEmpty(value) ? null : (Guid?)new Guid(value);
        }
        catch
        {
            return null;
